      ...
      <div class="phone-container" id="container">
        <div class="phone-screen bg-white" id="main-screen">
            ...
            <div class="page active px-4 pt-4" id="page-home">
                <div class="flex justify-between items-center mb-6">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full overflow-hidden border-2 border-blue-100 bg-blue-50 flex items-center justify-center cursor-pointer"
                            onclick="switchPage('profile')">
                            <img v-if="avatar_url" :src="avatar_url" class="w-full h-full object-cover">
                            <iconify-icon v-else icon="mdi:user" class="text-2xl text-blue-400"></iconify-icon>
                        </div>
                        <div>
                            ...
                            <span class="text-[9px] px-1.5 py-0.5 rounded-full font-medium"
                                :class="userInfo.user_type == 'normal' ? 'bg-blue-100 text-blue-600' : 'bg-orange-100 text-orange-600'">
                                {{userInfo.user_type == 'normal' ? '正式会员' : '试用会员'}}
                            </span>
                        </div>
                    </div>
                    <div class="flex items-center gap-4">
                        <div class="relative cursor-pointer h-[20px]" onclick="switchPage('notifications')">
                            <iconify-icon class="text-xl text-slate-600" icon="lucide:bell"></iconify-icon>
                            <span v-if="unread_count > 0"
                                class="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full text-white text-[9px] flex items-center justify-center">{{unread_count > 9 ? '9+' : unread_count}}</span>
                        </div>
                        <iconify-icon onclick="switchPage('profile')" class="text-xl text-slate-600"
                            icon="uil:setting"></iconify-icon>
                    </div>
                </div>
                <div>
                <p class="text-xl font-bold mb-4">欢迎</p>
</div>
                <div class="grid grid-cols-4 gap-2 mb-4">
                    <div class="bg-blue-50 rounded-lg p-3 text-center">
                        <div class="text-lg font-bold text-blue-600">{{userStats.total}}</div>
                        <div class="text-[10px] text-slate-500">刷题总数</div>
                    </div>
                    <div class="bg-green-50 rounded-lg p-3 text-center">
                        <div class="text-lg font-bold text-green-600">{{userStats.total > 0 ? (userStats.correct / userStats.total * 100).toFixed(0) + '%' : '--'}}</div>
                        <div class="text-[10px] text-slate-500">正确率</div>
                    </div>
                    <div class="bg-purple-50 rounded-lg p-3 text-center">
                        <div class="text-lg font-bold text-purple-600">{{userStats.papers}}</div>
                        <div class="text-[10px] text-slate-500">完成试卷</div>
                    </div>
                    <div class="bg-orange-50 rounded-lg p-3 text-center">
                        <div class="text-lg font-bold text-orange-600">{{userStats.days}}</div>
                        <div class="text-[10px] text-slate-500">刷题天数</div>
                    </div>
                </div>
                <button onclick="switchPage('history')"
                    class="w-full mb-4 py-2 border border-slate-200 rounded-sm text-sm text-slate-500 flex items-center justify-center gap-2">
                    <iconify-icon icon="lucide:history" class="text-base"></iconify-icon>
                    查看刷题记录
                </button>
                <div v-if="weakPoints.length > 0" class="rounded-lg border border-orange-100 bg-orange-50 p-4 mb-4">
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center gap-2">
                            <iconify-icon icon="lucide:target" class="text-orange-500 text-lg"></iconify-icon>
                            <span class="text-sm font-semibold text-slate-700">薄弱知识点</span>
                        </div>
                        <span @click="goToKnowledgeView()" class="text-xs text-orange-500 cursor-pointer">查看详情 ›</span>
                    </div>
                    <div class="space-y-2 mb-3">
                        <div v-for="(kp, idx) in weakPoints" :key="idx" class="flex items-center gap-2">
                            <span class="text-xs text-slate-600 shrink-0 truncate" style="width:5rem">{{kp.name}}</span>
                            <div class="flex-1 bg-orange-100 rounded-full h-1.5 overflow-hidden">
                                <div class="bg-orange-400 h-full rounded-full" :style="{width: (kp.count / weakPoints[0].count * 100) + '%'}"></div>
                            </div>
                            <span class="text-xs text-orange-600 font-bold shrink-0" style="width:2rem;text-align:right">{{kp.count}}</span>
                        </div>
                    </div>
                    <div class="text-xs text-orange-700 bg-white rounded px-3 py-2 border border-orange-100">
                        💡 建议重点练习「{{weakPoints[0].name}}」，该知识点共答错 {{weakPoints[0].count}} 道题
                    </div>
                </div>
                <div class="bg-blue-500 p-4 rounded-md mb-4 text-white cursor-pointer shadow"
                    @click="toDailyPractice()">
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="flex items-center gap-2 mb-1">
                                <iconify-icon icon="lucide:calendar-check" class="text-lg"></iconify-icon>
                                <span class="text-xl font-semibold">每日一练</span>
                            </div>
                            <div class="text-sm opacity-75 mb-2">{{dailyDate}} · 每日10道精选题</div>
                            <span class="text-sm bg-white/20 px-2 py-0.5 rounded-full">
                                {{dailyDone ? '今日已完成 ✓' : (dailyHasProgress ? '继续上次进度' : '今日未开始')}}
                            </span>
                        </div>
                        <iconify-icon icon="lucide:chevron-right" class="text-2xl opacity-70"></iconify-icon>
                    </div>
                </div>
                <div class="mb-6">
                    <h3 class="text-base font-semibold text-slate-700 mb-2">套卷练习</h3>
                    <div class="space-y-3">
                        <select class="bg-slate-50 p-3 w-full text-base text-slate-700" v-model="chosenRange">
                            <option value="">请选择套卷</option>
                            <option :value="item.Id" v-for="(item, index) in filteredCollections" :key="index">
                                {{item.collection_fullname}}</option>
                        </select>
                        <div v-if="filteredCollections.length == 0 && userInfo.user_type != 'normal'"
                            class="text-xs text-orange-500 text-center py-1">试用版暂无可用套卷，升级正式会员后可解锁</div>
            ...
            <div class="page min-h-full bg-white" id="page-mistake-history">
                <div class="review-header px-4 py-4 pb-0 border-b-neutral-300 border-b-[1px] sticky w-full top-0 left-0 bg-white z-[10]">
                    <div class="flex border-[1px] border-blue-600 rounded-sm mb-3">
                        <div class="flex-1 py-1 text-sm text-center"
                            :class="mistake_view_mode == 'type' ? 'bg-blue-600 text-white' : 'text-blue-600'"
                            @click="mistake_view_mode = 'type'">按题型</div>
                        <div class="flex-1 py-1 text-sm text-center"
                            :class="mistake_view_mode == 'knowledge' ? 'bg-blue-600 text-white' : 'text-blue-600'"
                            @click="mistake_view_mode = 'knowledge'">按知识点</div>
                        <div class="flex-1 py-1 text-sm text-center"
                            :class="mistake_view_mode == 'favorite' ? 'bg-blue-600 text-white' : 'text-blue-600'"
                            @click="mistake_view_mode = 'favorite'">我的收藏</div>
                        <div class="flex-1 py-1 text-sm text-center"
                            :class="mistake_view_mode == 'marked' ? 'bg-blue-600 text-white' : 'text-blue-600'"
                            @click="mistake_view_mode = 'marked'">疑难问题</div>
                    </div>
                    <div class="flex pt-2 mb-[-1px]" v-if="mistake_view_mode == 'type'">
                        <div class="flex-1 py-1 text-sm text-center border-b "
                            :class="active_history_type == '' ? 'text-blue-600 font-bold border-blue-600' : 'border-transparent'"
                            @click="active_history_type = ''">全部({{mistake_history.length}})</div>
                        <div class="flex-1 py-1 text-sm text-center border-b "
                            :class="active_history_type == '单选' ? 'text-blue-600 font-bold border-blue-600' : 'border-transparent'"
                            @click="active_history_type = '单选'">单选({{mistake_history_cate.single}})</div>
                        <div class="flex-1 py-1 text-sm text-center border-b"
                            :class="active_history_type == '多选' ? 'text-blue-600 font-bold border-blue-600' : 'border-transparent'"
                            @click="active_history_type = '多选'">多选({{mistake_history_cate.multi}})</div>
                        <div class="flex-1 py-1 text-sm text-center border-b "
                            :class="active_history_type == '判断' ? 'text-blue-600 font-bold border-blue-600' : 'border-transparent'"
                            @click="active_history_type = '判断'">判断({{mistake_history_cate.tof}})</div>
                    </div>
                    <div class="flex pt-2 mb-[-1px]" v-if="mistake_view_mode == 'marked'">
                        <div class="flex-1 py-1 text-sm text-center border-b"
                            :class="active_history_type == '' ? 'text-blue-600 font-bold border-blue-600' : 'border-transparent'"
                            @click="active_history_type = ''">全部({{markedQuestionList.length}})</div>
                        <div class="flex-1 py-1 text-sm text-center border-b"
                            :class="active_history_type == '单选' ? 'text-blue-600 font-bold border-blue-600' : 'border-transparent'"
                            @click="active_history_type = '单选'">单选({{marked_cate.single}})</div>
                        <div class="flex-1 py-1 text-sm text-center border-b"
                            :class="active_history_type == '多选' ? 'text-blue-600 font-bold border-blue-600' : 'border-transparent'"
                            @click="active_history_type = '多选'">多选({{marked_cate.multi}})</div>
                        <div class="flex-1 py-1 text-sm text-center border-b"
                            :class="active_history_type == '判断' ? 'text-blue-600 font-bold border-blue-600' : 'border-transparent'"
                            @click="active_history_type = '判断'">判断({{marked_cate.tof}})</div>
                    </div>
                </div>
                <div class="" v-for="item in weeked_history"></div>
                <!-- 按题型 视图 -->
                <div class="mistake-list flex flex-col" v-if="mistake_view_mode == 'type'">
                    <div class="p-4 text-center text-base" v-if="filtered_history.length == 0">
                        暂无数据
                    </div>
                    <div v-for="(value, key) in weeked_history" :key="key"
                        class="border-b-[1px] border-neutral-300">
                        <div class="p-4 flex items-center justify-between cursor-pointer"
                            @click="toggleHistoryGroup(key)">
                            <div class="flex items-center gap-2">
                                <span class="inline-block h-[14px] w-[3px] rounded-full bg-blue-400"></span>
                                <span class="text-base font-semibold text-slate-800">{{key}}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="text-sm text-slate-500 bg-blue-50 px-2 py-0.5 rounded-full">{{value.length}}题</span>
                                <iconify-icon
                                    :icon="expanded_history[key] ? 'lucide:chevron-up' : 'lucide:chevron-down'"
                                    class="text-lg text-slate-400"></iconify-icon>
                            </div>
                        </div>
                        <div v-if="expanded_history[key]" class="bg-slate-50">
                            <div v-for="(item, iIndex) in value" :key="iIndex"
                                class="px-4 py-3 border-t border-neutral-200 flex items-start gap-2">
                                <div class="flex-1" @click="showRemark(item)">
                                    <div class="text-neutral-600 text-xs">{{item.answer_time}}</div>
                                    <div class="text-sm text-slate-800 pt-1">{{item.question_name}}</div>
                                    <div class="text-neutral-500 text-xs pt-1" v-if="item.question_type != '判断'">
                                        <span class="text-green-500">正确答案：{{item.answer_correct}}</span>
                                        <span class="mx-2 text-neutral-300">|</span>
                                        <span class="text-red-500">您选择：{{item.answer_user}}</span>
                                    </div>
                                    <div class="text-neutral-500 text-xs pt-1" v-else>
                                        <span class="text-green-500">正确答案：{{item.answer_correct == 'A' ? '对' : '错'}}</span>
                                        <span class="mx-2 text-neutral-300">|</span>
                                        <span class="text-red-500">您选择：{{item.answer_user == 'A' ? '对' : '错'}}</span>
                                    </div>
                                    <div class="text-neutral-500 text-xs pt-1">点击查看解析</div>
                                </div>
                                <button @click="toggleFavorite(item)" class="shrink-0 pt-1">
                                    <iconify-icon
                                        :icon="isFavorited(item.qiestion_id) ? 'mdi:heart' : 'mdi:heart-outline'"
                                        class="text-xl"
                                        :class="isFavorited(item.qiestion_id) ? 'text-red-500' : 'text-neutral-300'">
                                    </iconify-icon>
                                </button>
                            </div>
                        </div>
                ...
                <!-- 按知识点 视图 -->
                <div class="flex flex-col" v-if="mistake_view_mode == 'knowledge'">
                    <div class="p-4 text-center text-base" v-if="mistake_history_by_knowledge.length == 0">
                        暂无数据
                    </div>
                    <div v-for="(group, gIndex) in mistake_history_by_knowledge" :key="group.name"
                        class="border-b-[1px] border-neutral-300">
                        <div class="p-4 pb-2 flex items-center justify-between cursor-pointer"
                            @click="toggleKpGroup(group.name)">
                            <div class="flex items-center gap-2">
                                <span class="inline-block h-[14px] w-[3px] rounded-full bg-orange-400"></span>
                                <span class="text-base font-semibold text-slate-800">{{group.name}}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="text-sm text-orange-600 font-bold">{{group.items.length}}题</span>
                                <iconify-icon
                                    :icon="expanded_kp[group.name] ? 'lucide:chevron-up' : 'lucide:chevron-down'"
                                    class="text-lg text-slate-400"></iconify-icon>
                            </div>
                        </div>
                        <div class="px-4 pb-3">
                            <div class="w-full bg-orange-100 rounded-full h-1.5 overflow-hidden">
                                <div class="bg-orange-400 h-full rounded-full" :style="{width: (group.items.length / maxKpCount * 100) + '%'}"></div>
                            </div>
                        </div>
                        <div v-if="expanded_kp[group.name]" class="bg-slate-50">
                ...
                <!-- 我的收藏 视图 -->
                <div class="mistake-list flex flex-col" v-if="mistake_view_mode == 'favorite'">
                    <div class="p-4 text-center text-base" v-if="filtered_favorites.length == 0">
                        暂无收藏
                    </div>
                    <div v-for="(item, index) in filtered_favorites"
                        class="panel-block border-b-[1px] p-4 border-neutral-300 flex items-start gap-2">
                        <div class="flex-1" @click="showRemark(item)">
                            <div class="text-neutral-600 text-xs">{{item.answer_time}}</div>
                            <div class="text-sm text-slate-800 pt-1">{{item.question_name}}</div>
                            <div class="text-neutral-500 text-xs pt-1" v-if="item.question_type != '判断'">
                ...
                <div class="flex flex-col" v-if="mistake_view_mode == 'marked'">
                    <div class="p-4 text-center text-base" v-if="filtered_marked.length == 0">
                        暂无数据
                    </div>
                            <div v-for="(item, iIndex) in filtered_marked" :key="iIndex"
                                class="px-4 py-3 border-t border-neutral-200 flex items-start gap-2">
                                <div class="flex-1" @click="showRemarkMarked(item)">
                                    <div class="text-neutral-600 text-xs">{{item.createdTime.year + 1900}}-{{item.createdTime.month + 1 >= 10? item.createdTime.month + 1 : '0' + (item.createdTime.month + 1)}}-{{item.createdTime.date >= 10? item.createdTime.date : '0' + item.createdTime.date}} {{item.createdTime.hours}}:{{item.createdTime.minutes >= 10? item.createdTime.minutes : '0' + item.createdTime.minutes}}:{{item.createdTime.seconds >= 10? item.createdTime.seconds : '0' + item.createdTime.seconds}}</div>
                                    <div class="text-sm text-slate-800 pt-1">{{item.question_title}}</div>
                                    <div class="text-neutral-500 text-xs pt-1" v-if="item.question_type != '判断'">
                                        <span class="text-green-500">正确：{{item.answer_correct}}</span>
                                    </div>
                                    <div class="text-neutral-500 text-xs pt-1" v-else>
                                        <span class="text-green-500">正确：{{item.answer_correct == 'A' ? '对' : '错'}}</span>
            ...
            <div class="page min-h-full bg-white" id="page-mistakes">
                ...
                <div class="mistake-list flex flex-col">
                    ...
                    <div v-for="(item, index) in filtered_mistakes" @click="toMistakeBefore(item)"
            ...
            <div class="page px-4 pt-4 min-h-full bg-white" v-loading="questLoad" id="page-quiz">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <div class="flex-1 text-right flex justify-end">
                        <div class="flex items-center text-xs" :class="questMarked? 'text-blue-600' : ''" @click="markDecide()">
                        <iconify-icon icon="hugeicons:question" class="text-xl"  ></iconify-icon> 
                         {{questMarked? '已标记': '标记疑难'}}</div>
                    </div>
                </div>
                ...
                <div class="analyze-sect mt-4">
                    ...
                    <div class="flex gap-4 mt-4">
                        <button class="flex-1 py-2 bg-blue-600 text-white font-semibold text-sm"
                            @click="retryQuest()" v-if="!(info.mode == 2 && statistic.total_score == 100)">重试</button>
                        <button class="flex-1 py-2 bg-blue-600 text-white font-semibold text-sm"
                            onclick="switchPage('home')">回到主页</button>
                    </div>
            </div>
            <div class="page px-4 pt-4 min-h-full bg-white" id="page-login">
                <div id="loginForm" class="p-6 space-y-4 bg-white">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <span class="absolute left-3 top-[calc(50%+1.5px)] -translate-y-1/2 text-gray-400">
                                <iconify-icon icon="solar:lock-password-line-duotone" class="text-base"></iconify-icon>
                            </span>
            ...
            <div class="page px-4 pt-6 min-h-full bg-white" id="page-profile">
            <div class="flex justify-between items-center">
                    <div class="flex items-center flex-1">
                        <button class="text-slate-400 me-2" style="height: 20px;" onclick="switchPage('home')">
                            <iconify-icon class="text-xl" icon="lucide:x"></iconify-icon>
                ...
                <div class="flex flex-col items-center pb-6 mb-4 border-b border-slate-100">
                    <div class="cursor-pointer mb-1" @click="showAvatarModal=true">
                        <div class="w-20 h-20 rounded-full bg-blue-50 flex items-center justify-center border-2 border-blue-100 overflow-hidden relative">
                            <img v-if="avatar_url" :src="avatar_url" class="w-full h-full object-cover">
                            <iconify-icon v-else icon="mdi:user" class="text-5xl text-blue-600"></iconify-icon>
                            <div class="absolute inset-0 bg-black/30 flex items-center justify-center rounded-full">
                                <iconify-icon icon="mdi:camera-plus" class="text-2xl text-white"></iconify-icon>
                            </div>
                        </div>
                        <p class="text-xs text-center text-slate-400 mt-1">点击更换头像</p>
                    </div>
                    <h2 class="text-xl font-bold text-slate-800">{{userInfo.name || '未设置姓名'}}</h2>
                    <p class="text-sm text-slate-500 mt-1">{{userInfo.phone}}</p>
                    <span class="mt-2 text-[10px] px-2 py-0.5 rounded-full font-medium"
                        :class="userInfo.user_type == 'normal' ? 'bg-blue-100 text-blue-600' : 'bg-orange-100 text-orange-600'">
                        {{userInfo.user_type == 'normal' ? '正式会员' : '试用会员'}}
                    </span>
                </div>
                <div class="space-y-4">
                    <div class="p-4 pb-0">
                        <div class="flex items-center gap-1 mb-3">
                            <iconify-icon icon="mdi:account-edit-outline" class="text-xl text-blue-600"></iconify-icon>
                            <span class="text-lg font-semibold text-slate-600">修改姓名</span>
                        </div>
                        <div class="flex gap-2">
                            <input type="text" v-model="profileForm.name"
                                class="flex-1 text-base px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                :placeholder="userInfo.name || '请输入新姓名'">
                            <button @click="updateUserName()"
                                class="px-4 py-2 bg-blue-600 text-white text-sm font-semibold shrink-0">保存</button>
                        </div>
                    </div>
                    <div class="p-4">
                        <div class="flex items-center gap-1 mb-3">
                            <iconify-icon icon="solar:lock-password-line-duotone" class="text-xl text-blue-600"></iconify-icon>
                            <span class="text-lg font-semibold text-slate-600">修改密码</span>
                        </div>
                        <div class="space-y-2">
                            <div class="relative">
                                <span class="absolute left-3 top-[calc(50%+1px)] -translate-y-1/2 text-gray-400">
                                    <iconify-icon icon="solar:lock-password-line-duotone" class="text-base"></iconify-icon>
                                </span>
                                <input type="password" v-model="profileForm.oldPassword"
                                    class="w-full pl-10 text-base pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                    placeholder="当前密码">
                            </div>
                            <div class="relative">
                            ...
                            <button @click="changePassword()"
                                class="w-full py-2 bg-blue-600  text-white text-base font-semibold">确认修改</button>
                        </div>
                    </div>
                    <div class="px-4 pb-4">
                    <button @click="doLogOut()"
                        class="w-full py-2 border border-red-200 text-red-500 text-base font-semibold">退出登录</button>
                    </div>
                    
                </div>
            </div>
            <div class="page min-h-full bg-white" id="page-history">
                <div class="sticky top-0 bg-white z-10 px-4 py-4 border-b border-neutral-200 flex items-center gap-3">
                    <button class="text-slate-400" style="height: 20px;" onclick="switchPage('home')">
                        <iconify-icon class="text-xl" icon="lucide:chevron-left"></iconify-icon>
                    </button>
                    <span class="text-base font-semibold text-slate-800">刷题记录</span>
                </div>
                <div class="flex flex-col">
                    <div class="p-8 text-center text-sm text-slate-400" v-if="quizHistory.length == 0">暂无刷题记录</div>
                    <div v-for="(value, key) in weeked_quiz_history" :key="key" class="border-b border-neutral-300">
                        <div class="p-4 flex items-center justify-between cursor-pointer" @click="toggleQuizHistoryGroup(key)">
                            <div class="flex items-center gap-2">
                                <span class="inline-block h-[14px] w-[3px] rounded-full bg-blue-400 shrink-0"></span>
                                <span class="text-base font-semibold text-slate-800">{{key}}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="text-sm text-slate-500 bg-blue-50 px-2 py-0.5 rounded-full">{{value.length}}次</span>
                                <iconify-icon :icon="expanded_history_quiz[key] ? 'lucide:chevron-up' : 'lucide:chevron-down'" class="text-lg text-slate-400"></iconify-icon>
                            </div>
                        </div>
                        <div v-if="expanded_history_quiz[key]" class="bg-slate-50">
                            <div v-for="(item, index) in value" :key="index" class="border-t border-neutral-200 p-4">
                                <div class="text-neutral-400 text-xs">{{item.c_time}}</div>
                                <div class="flex items-center justify-between pt-1">
                                    <div class="flex-1 min-w-0 mr-3">
                                        <div class="flex items-center text-sm flex-wrap gap-1">
                                            <span class="shrink-0 bg-blue-100 text-blue-600 font-semibold py-0.5 px-2 rounded-sm text-xs">
                                                {{item.array_type == 0 ? '题库' : '套卷'}}
                                            </span>
                                            <span class="truncate text-slate-800">{{item.array_name}}</span>
                                        </div>
                                        <div class="text-neutral-400 text-xs pt-1 flex gap-3">
                                            <span v-if="item.spent_time">用时 {{item.spent_time}}</span>
                                            <span>答错 <span class="text-red-500">{{item.mistake_list ? item.mistake_list.split(',').filter(function(v){return v}).length : 0}}</span> 题</span>
                                        </div>
                                    </div>
                                    <div class="text-blue-500 border-2 border-blue-400 w-10 h-10 rounded-full flex flex-col items-center justify-center shrink-0">
                                        <span class="text-[11px] font-bold leading-tight">{{parseFloat(item.final_score) % 1 > 0 ? parseFloat(item.final_score).toFixed(1) : item.final_score}}%</span>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="page min-h-full bg-white" id="page-notifications">
                <div class="sticky top-0 bg-white z-10 px-4 py-4 border-b border-neutral-200 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <button class="text-slate-400" style="height: 20px;" onclick="switchPage('home')">
                            <iconify-icon class="text-xl" icon="lucide:chevron-left"></iconify-icon>
                        </button>
                        <span class="text-base font-semibold text-slate-800">消息通知</span>
                    </div>
                    <button v-if="notifications.length > 0" @click="markAllNotificationsRead()"
                        class="text-sm text-blue-600">全部已读</button>
                </div>
                <div class="flex flex-col">
                    <div class="p-8 text-center text-sm text-slate-400" v-if="notifications.length == 0">暂无消息</div>
                    <div v-for="(item, index) in notifications" :key="index"
                        @click="markNotificationRead(item)"
                        class="border-b border-neutral-100 p-4 cursor-pointer"
                        :class="item.is_read == 0 ? 'bg-blue-50' : 'bg-white'">
                        <div class="flex items-start gap-3">
                            <div class="shrink-0 mt-[6px]">
                                <div class="w-2 h-2 rounded-full" :class="item.is_read == 0 ? 'bg-blue-500' : 'bg-transparent'"></div>
                            </div>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between gap-2">
                                    <span class="text-sm font-semibold text-slate-800 truncate">{{item.msg_title}}</span>
                                    <span class="text-xs text-slate-400 shrink-0">{{item.c_time}}</span>
                                </div>
                                <p class="text-sm text-slate-600 mt-1 leading-relaxed">{{item.msg_content}}</p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div v-if="showAvatarModal"
             style="position:fixed;inset:0;z-index:200;background:rgba(0,0,0,0.5);display:flex;align-items:flex-end"
             @click.self="showAvatarModal=false">
            <div style="width:100%;background:#fff;border-radius:20px 20px 0 0;overflow:hidden">
                <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100">
                    <span class="text-base font-semibold text-slate-700">更换头像</span>
                    <button @click="showAvatarModal=false">
                        <iconify-icon icon="lucide:x" class="text-xl text-slate-400"></iconify-icon>
                    </button>
                </div>
                <iframe id="avatar-upload-frame" :src="'https://www.wiidu.com.cn/zx/customForm/formPreview.html?a=1&tblname=custom_pfp_upd&R=26758.48778610215&userid='+userid"
                        style="width:100%;height:320px;border:none;display:block"></iframe>
            </div>
        </div>
        <div class="absolute bottom-0 left-0 right-0 h-20 bg-white/80 backdrop-blur-md border-t border-slate-100 flex items-center justify-around px-2 z-50 rounded-b-[40px]"
            id="bottom-nav" style="height: 5rem">
            ...
            <div class="nav-item flex flex-col items-center gap-1 cursor-pointer transition-all"
                onclick="switchPage('sp-video')">
                <iconify-icon class="text-xl" icon="tabler:video"></iconify-icon>
                <span class="text-[10px] font-bold">视频专区</span>
            </div>
<!--            <div class="nav-item flex flex-col items-center gap-1 cursor-pointer transition-all"-->
<!--                onclick="switchPage('profile')">-->
